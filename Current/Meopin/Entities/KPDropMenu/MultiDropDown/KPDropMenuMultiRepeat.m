...
#import "KPDropMenuMultiRepeat.h"
#import "MultiHeaderView.h"

@interface KPDropMenuMultiRepeat () <UITableViewDelegate, UITableViewDataSource> {
...
@implementation KPDropMenuMultiRepeat
...
- (void)initLayer {
    ...
    _arrIndexItems =  [NSMutableArray array];
    SelectedIndex = -1;
    isCollapsed = TRUE;
...
- (void)btnDoneSelectClick {
    ...
    if(_delegate != nil){
        if([_delegate respondsToSelector:@selector(didSelectItem:arrSelectedIndex:)])
            [_delegate didSelectItem:self arrSelectedIndex:_arrIndexItems];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    int checkIndex = (int)indexPath.row;
    if ([_arrIndexItems containsObject:[NSNumber numberWithInt:checkIndex]]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_arrIndexItems containsObject:[NSNumber numberWithInt:SelectedIndex]]) {
        [_arrIndexItems removeObject:[NSNumber numberWithInt:SelectedIndex]];
    }
    else {
        [_arrIndexItems addObject:[NSNumber numberWithInt:SelectedIndex]];
    }
