//
//  PT_WriteReviewStep4VC.swift
//  Meopin
//
//  Created by Tops on 11/9/17.
...
class PT_WriteReviewStep4VC: UIViewController {
    ...
    @IBOutlet var lblCheckMarkDetail: UILabel!
    ...
    override func viewDidLoad() {
        ...
        if (shareObjReview.arrReviewGroupList.count > 2) {
            let shareObjGroup = shareObjReview.arrReviewGroupList[2]
    ...
    func setLanguageTitles() {
        ...
        lblCheckMarkDetail.text = Global().getLocalizeStr(key: "keyRVWCheckMarkDetail")
        ...
        btnNext.setTitle(Global().getLocalizeStr(key: "keyRVWNextAndFinalStep"), for: .normal)
    ...
    func setDeviceSpecificFonts() {
        ...
        lblCheckMarkDetail.font = UIFont(name: Global.kFont.SourceRegular, size: Global.singleton.getDeviceSpecificFontSize(12))
...
//MARK: -
extension PT_WriteReviewStep4VC {
    // MARK: -  API Call
...
//            }
            ...
            let pt_writeReviewStep5Obj = PT_WriteReviewStep5VC(nibName: "PT_WriteReviewStep5VC", bundle: nil)
            pt_writeReviewStep5Obj.strSelProviderId = self.strSelProviderId
            pt_writeReviewStep5Obj.dictSelProviderData = self.dictSelProviderData
            pt_writeReviewStep5Obj.isNavFavScreen = self.isNavFavScreen
            pt_writeReviewStep5Obj.shareObjReview = self.shareObjReview
            self.navigationController?.pushViewController(pt_writeReviewStep5Obj, animated: true)
        }
    }
    
    func btnCheckNoInfoClick(_ sender: UIButton) {
        let touchPoint: CGPoint = sender.convert(CGPoint.zero, to: self.tblGroupQuestion)
        let indexPath = tblGroupQuestion.indexPathForRow(at: touchPoint)
        let cell: Pt_WriteReviewQueCell = tblGroupQuestion.cellForRow(at: indexPath!) as! Pt_WriteReviewQueCell
        ...
        if (shareObjReview.arrReviewGroupList.count > 2) {
            ...
            if (sender.tag == 0) {
                sender.tag = 1
                shareObjQue.intRWQueRating = -1
                
                cell.tpRatingView.fullSelectedImage = #imageLiteral(resourceName: "imgRate0")
                cell.lblRWAnswer.text = Global().getLocalizeStr(key: "keyRVWNoInformation")
                cell.btnCheckNoInfo.setImage(#imageLiteral(resourceName: "imgCheckmarkSel"), for: .normal)
                cell.tpRatingView.isUserInteractionEnabled = false
                ...
                cell.tpRatingView.fullSelectedImage = #imageLiteral(resourceName: "imgRate0")
                cell.btnCheckNoInfo.setImage(#imageLiteral(resourceName: "imgCheckmark"), for: .normal)
                cell.lblRWAnswer.text = "\(Global().getLocalizeStr(key: "keyRVWYourAnswer")) \(shareObjQue.intRWQueRating)"
                cell.tpRatingView.isUserInteractionEnabled = true
...
// MARK: -
extension PT_WriteReviewStep4VC: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.btnCheckNoInfo.addTarget(self, action: #selector(btnCheckNoInfoClick(_ :)), for: .touchUpInside)
        
        cell.btnCheckNoInfo.isHidden = false
        cell.btnCheckNoInfo.setImage(#imageLiteral(resourceName: "imgCheckmark"), for: .normal)
        if (shareObjReview.arrReviewGroupList.count > 2) {
            let shareObjGroup = shareObjReview.arrReviewGroupList[2]
            ...
            if (shareObjQue.intRWQueRating == -1) {
                cell.tpRatingView.fullSelectedImage = #imageLiteral(resourceName: "imgRate0")
                cell.lblRWAnswer.text = Global().getLocalizeStr(key: "keyRVWNoInformation")
                cell.btnCheckNoInfo.setImage(#imageLiteral(resourceName: "imgCheckmarkSel"), for: .normal)
                cell.tpRatingView.isUserInteractionEnabled = false
            }
            else if (shareObjQue.intRWQueRating == 0) {
                cell.tpRatingView.fullSelectedImage = #imageLiteral(resourceName: "imgRate0")
                cell.lblRWAnswer.text = "\(Global().getLocalizeStr(key: "keyRVWYourAnswer")) \(shareObjQue.intRWQueRating)"
            }
...
//MARK: -
extension PT_WriteReviewStep4VC : TPFloatRatingViewDelegate {
    ...
    func calculateTotalRating() {
        if (shareObjReview.arrReviewGroupList.count > 2) {
            ...
            if (intTotCount == 0) {
                self.lblScoreRating.text = "0"
            }
            else {
                self.lblScoreRating.text = String(format: "%.1f", floatTotRating/Float(intTotCount))
            }
...
//MARK: -
extension PT_WriteReviewStep4VC : MDHTMLLabelDelegate {
...
//MARK: -
extension PT_WriteReviewStep4VC : UITextViewDelegate {
